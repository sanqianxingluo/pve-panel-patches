...
| **系统更新** | System → PVE 工具集 | 检查更新 / 更新 PVE 软件 / 更新内核；更新前自动保留旧内核，可一键回退 |
...
- **硬盘概要** —— 型号 + 温度 + 健康 + 寿命 + 累计读写：
  `Lexar SSD NM620 512GB 39 °C 正常 寿命 95% 读 13.7 TB 写 19.0 TB`
  （温度 NVMe 走 hwmon、SATA 走 `smartctl`；健康/寿命/读写由 root 定时采集到缓存，
  面板只读缓存 —— 因为接口以 `www-data` 执行，而 `smartctl` 读 `/dev/*` 需要 root）
...
### 2.3 系统更新（更新 PVE / 更新内核，旧内核可回退）

「PVE 工具集」页底部的**系统更新**分区，五个按钮：

| 按钮 | 做什么 |
|---|---|
| **检查更新** | 联机刷新一次软件源索引，数出可升级的 PVE 包与内核包各多少 |
| （下面两个「更新」按钮都要过**两道确认关**） | 先弹确认操作，确认后再弹**红色警告**，点「我已知悉」才真正执行 |
| **更新 PVE 软件** | 升级 PVE 与 Debian 的软件包。**内核一枚都不动** |
| **更新内核** | 装上源里最新的内核，并把启动项切到它 |
| **回退到旧内核** | 把启动项切回更新前保留的那枚内核 |
| **确认新内核正常，解除保留** | 确认没问题后，解除对旧内核的保留 |

#### 两道确认关

「更新 PVE 软件」与「更新内核」都是**改宿主的**操作，所以做成两级确认：

1. **确认操作** —— 普通确认框，说明这次会做什么、内核怎么保。
2. **红色警告** —— 红色标题栏 + 红色警示文字的警告框，写明「这会真实改动宿主系统」、
   需要重启的会写明、用到 DKMS 的会写明「不在本工具处理范围」。
   这一步必须点 **「我已知悉」** 才会真的执行；点「取消」或右上角关闭都当放弃。

只有这两关都过了，才会真正动宿主包。

#### 回退是怎么保住的

点「更新 PVE 软件」或「更新内核」时，**动手前**会自动做一件事：
把**当前正在运行的那枚内核**用 `apt-mark hold` 钉住，并记下它的包名。

- 「钉住」有两个作用：它不会被顺手升级或卸掉，也就不会被 `autoremove` 清走。
- 这套工具**永不执行 `apt autoremove`，永不删内核**。
- 面板上因此始终显示「保留的旧内核：<版本> 已保留，可回退」。

更新完、重启进新内核、确认一切正常之后，再点「解除保留」。
那时若启动项还钉在旧内核上，会**顺手切回最新内核**，避免「解除了保留却还在跑旧的」。

> 若某一刻发现旧内核被钉得**不完整**（面板会写「保留不完整 (n/m)」），
> 说明有个包没钉上，这种状态回退可能失败 —— 面板会明确标出来，不假装没事。

#### 关于重启

**这套功能不会自动重启，也不替你重启。** 内核更新必须重启才生效，
重启由你自己在方便的时候做。

> **本机就跑在宿主的一台 VM 里时要注意**：重启宿主的瞬间，本机（以及这次会话）
> 会随之中断。重启前把基础设施类的 VM/CT 都设成 `onboot=1`，
> 否则宿主起来了、它们还躺着。

#### 核显 SR-IOV / DKMS 不在本工具处理范围

用了核显 SR-IOV（`i915-sriov-dkms`）这类 DKMS 模块的机器，
**更新内核后驱动需要为新内核重新编译**。这件事**不在本工具的处理范围内** ——
本工具只做「更新 PVE、更新内核、保留旧内核可回退」，不代为编译、不代为干预。

面板上只如实提示这一句。回退仍是有效的退路：切回旧内核 + 重启，
旧内核上原有的 DKMS 模块照旧可用。
...
| `fan-scan` | 查看探测缓存 |
| `upgrade-status` | 更新状态：当前/已装/可回退内核、可升级包计数 |
| `upgrade-check` | 联机刷新源索引并统计可升级的 PVE / 内核包 |
| `upgrade-pve [simulate]` | 更新 PVE 软件（内核不动）；`simulate` 只保留+校验、不安装 |
| `upgrade-kernel [simulate]` | 更新内核并保留旧内核 |
| `kernel-rollback` | 把启动项切回保留的旧内核 |
| `kernel-release` | 解除对旧内核的保留 |
| `upgrade-progress` | 查更新任务的进度 JSON |
...
最省事的做法是**用面板上的「更新内核」**（见 §2.3）：它会自动保留当前内核、
装上最新的、并把启动项切过去；不合适就点「回退到旧内核」切回来。
...
- **「硬盘温度」升级为「硬盘概要」**：除型号与温度外，新增**健康状态**（正常 / 警告 / 异常，带颜色）、**剩余寿命百分比**、**累计读取与写入量**。
  例：`Lexar SSD NM620 512GB 39 °C 正常 寿命 95% 读 13.7 TB 写 19.0 TB`
- **新增 root 定时采集器**（`/usr/local/bin/pve-hwtools-smart` + systemd timer，每 5 分钟刷新）：概要接口由 `pveproxy` 以 `www-data` 身份执行，而 `smartctl` 读 `/dev/*` **需要 root**（非 root 直接 `Permission denied`），健康/寿命/读写因此在面板里永远是空的。现在由 root 定时把 SMART 采到世界可读的缓存（`/run/pve-hwtools-smart.txt`），取样脚本只读缓存；缓存缺失或过期就留空，绝不报错。
- 采集器同时挂开机 oneshot —— `/run` 是内存盘，重启即空，不补这一步重启后头几分钟这些字段会是空的。
- NVMe 用 `Percentage Used` 折算剩余寿命、SATA 用厂商磨损属性（`Wear_Leveling_Count` / `Media_Wearout_Indicator`）；读写量 NVMe 取 `Data Units`、SATA 由 `Total_LBAs_* × 512` 折算。
- 容量与型号去重：型号里已含容量数字时不再单列（`…512GB` 后面不再跟 `512G`），否则这一行会被挤到换行。
- 型号里的字段分隔符（`|` `;`）会被剥掉，多块盘也不会把一行拆碎。

### V3.2 · 2026-09-26
- **概要面板改成半幅宽**：原来概要表格横向占满整个内容区（1121px），每行标签贴最左、数值贴最右，中间一大段空白，整块「看着很空」。现在概要面板只占半幅（560px），**CPU 图表自动流到右侧**、与概要保持同一行并底部齐平，整页不再有大片留白。
- 半幅后条目改为**每项整行**（占满半幅面板宽），与原生条目同风格；六项顺序：CPU温度 / 主板温度 / CPU核心温度 / 风扇转速 / CPU频率 / 硬盘温度。
- **面板高度改为按内容实测自适应**：原实现把高度写死（480 并按「隐藏行数 × 24px」估算），在半幅 + 整行条目下失准，会把最后一行裁掉。现在量取「最后一个可见条目」的底边算出应有高度，撑开/收回都准，开关任一条目都不会裁切或留空档。
- 顺带修掉一条**从未生效**的死代码：把面板高度钉成 480 的正则匹配的形态与宿主源码不符，一直没命中。

### V3.1 · 2026-09-26

**（已被 V3.2 取代）** 「硬件概要」条目由整幅改为半幅排列（一行两项）。方向对，但只改了条目宽度、没改面板本身宽度，整块仍占满内容区。V3.2 改为面板整体半幅，条目回归每项整行。

### V3.0 · 2026-09-25

**新增「系统更新」：更新 PVE、更新内核，旧内核可一键回退。**

面板「PVE 工具集」页底部新增一个分区，五个按钮：检查更新 / 更新 PVE 软件 /
更新内核 / 回退到旧内核 / 确认新内核正常，解除保留。

设计要点：

- **只在用户点按钮时才动作**，绝不自作主张，**绝不自动重启**。
- 动手前先把**当前运行内核** `apt-mark hold` 钉住并记下包名 —— 这就是「回退的退路」：
  它不会被升级或 `autoremove` 清走。
- **永不执行 `apt autoremove`，永不删内核。**
- 「更新 PVE 软件」只升软件，**内核一枚不动**（临时钉住全部内核包，结束后回收成「只钉运行内核」）。
- 「更新内核」装最新内核并把启动项切过去；旧内核保持钉住，随时可切回。
- 「回退到旧内核」把启动项切回保留的那枚（`proxmox-boot-tool kernel pin`），重启生效。
- 「解除保留」在确认新内核没问题后才用；若启动项还钉在旧内核上，顺便切回最新内核。
- 面板如实显示保留状态，**被钉得不完整时会明确标出**（回退可能失败），不假装没事。
- **核显 SR-IOV / DKMS 的驱动编译不在本工具处理范围**，面板只作提示。
- **更新操作两级确认**：先确认操作，再弹红色警告，点「我已知悉」才执行。

实测（走 HTTP 接口、ticket 认证）：保留内核 → 模拟校验 → 回退 → 解除保留 全链路通过，
每一步后机器的 hold、启动项、grub 默认项都核对过，最终完全复位。
