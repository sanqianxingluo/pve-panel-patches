...
- 主板温度
- 风扇转速（只列非零）
- 硬盘温度（NVMe 走 hwmon，SATA 走 smartctl）
- CPU 频率（实时 / 最小 / 最大）
...
| `/usr/share/pve-manager/js/pvemanagerlib.js` | 在 `PVE.node.StatusView` 的 items 里插入 `hw-*` 条目，面板高度改 540 |
...
# 预期：{"cpu_pkg":"46.0","cpu_cores":"45.0 43.0 ...","disks":"NVMe:39", ...}
```
