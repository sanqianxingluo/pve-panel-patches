...
**V3.1**（2026-09-26）
- **概要界面「硬件概要」改为半幅排列**：原来四项横跨整幅，标签在左、数值在右，中间大片留白、显得很空。现在每项占半幅、一行两项，与 PVE 原生条目的排版风格一致。
- 条目顺序微调为：CPU温度 / 主板温度 · CPU核心温度 / 风扇转速 · CPU频率 / 硬盘温度，让两行的内容各自相关、上下对齐。
- 多块硬盘的温度改为逐行换行列出（原为一行用竖线分隔，半幅下会被挤爆）。
