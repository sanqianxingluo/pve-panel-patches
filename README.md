...
| **占空比%** | 仅「手动定值」可用，**填百分比 1~100%**（下限锁 1%，绝不写 0 —— 0 就是停转） |
...
| **手动定值** | 固定占空比（**百分比 1~100%**），用于排查问题或固定风量 |
...
```
FAN1_CURVE=25:12,30:24,45:47,60:78,80:100
              ↑ 25°C 时 12%   ↑ 30°C 时 24%  …  ↑ 80°C 时 100%（满速）
```
...
**Q：换了主板 / 重新识别后通道数变了，界面会跟着变吗？**

会。界面**完全由探测结果驱动**，没有任何写死的通道数量：

- 通道清单是每次读取时**实时扫 `hwmon`** 得到的（本机就跳过了 6，只报 `1,2,3,4,5,7`）
- 设置页每次都**先清空再按探测结果重建**全部通道行，所以通道从 6 变 10 就显示 10 行
- 配置键也是随通道号生成的（`FAN8_*`、`SHOW_FANCH_10=…` 都接受），不会卡在 7
- 通道号**按数字排序**（1,2,…,9,10），不会出现「1,10,2,3」这种字典序

换主板后点「重新识别芯片与通道」即可；它会问你要不要保留通道名（新板子上通道号含义
往往全变，留旧名字容易张冠李戴），并重存原值基线。

**Q：为什么概要页显示的平均核心温度是 `-`？**
`sensors` 没有输出核心温度行（驱动没暴露 `Core N:` / `CPU N:`）。`sensors-detect --auto`
后一般就有了。
...
13. **风扇的硬件细节**（本机 NCT6798D / `nct6775`）：
    ...
    - 曲线五点温度单位是**毫摄氏度**，占空比百分比（写内核前换算成 `0-255`）
...
**改：风扇占空比改用百分比（1~100%），不再让人填 0~255 的裸值。**

界面上「占空比」原来是 `1~255` 的裸寄存器值 —— 见「50%」得自己心算 128，很不直观。
现在界面与配置文件**一律用百分比**，只有最底层写 `sysfs` 时才换算成 `0~255`。

- 界面：「占空比」输入框上限 255 → 100，「手动定值」「曲线五点」都是百分比
- 配置：`FANn_MANUAL=50`、`FANn_CURVE=25:12,30:24,45:47,60:78,80:100`
- **自动化迁移**：读到旧格式（任一值 > 100）就整条通道按 `0~255 → %` 换算，
  幂等、可反复执行。注意是**按通道整体**判定 —— 旧曲线 `25:30,30:60,45:120`
  里 30/60 也是裸值（真意 12%/24%），只挑 >100 的转会留下错值。
- 后端新增 `pwm_pct_to_raw` / `pwm_raw_to_pct`（四舍五入）；接口 `fans[].pwm` 也改报百分比
- 安全不变：下限仍是 1%，换算后另有一道 `raw<1 → 1` 的保险，**绝不写 0**
- 本机实测：填 50% 落成 `pwm=128`（1790 RPM）、填 100% 落成 `pwm=255`（3199 RPM），
  还原后原值 18 项逐字相符
...
通道清单由 shell 的 glob 展开得到，而 glob 是**字典序**：装 10 个通道的板子会排成
`1 10 2 3 4 5 6 7 8 9`，界面于是把「通道 10」排在「通道 1」正后面，`fan_hwid` 指纹
也跟着变成 `nct6798:ch1,10,2,…`。改成 `sort -n` 按数字排序。

本机只有 6 个通道（且跳过 6），所以这个 bug 一直没露头 —— 是用一个「假 hwmon 目录」
（10 通道，含 8/9/10）测出来的。顺带确认了三件事：探测、配置键校验、界面渲染**都是
随实际通道数变化的**，没有任何地方写死通道数量。
