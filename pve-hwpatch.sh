...
#
# 特性：幂等（已打过则跳过）；可反复执行；由 apt Post-Invoke 调用可自愈。
...
#
# 设计要点：/usr/bin/s.sh 只输出「纯 ASCII 数字」，单位（°C / RPM / MHz）一律由前端 JS 补，
...
# ---------- 1) 传感器取样脚本（纯 ASCII 数值）----------
if [ ! -f "$SH" ] || ! grep -q "$MARK-v4" "$SH" 2>/dev/null; then
  cat > "$SH" <<'EOS'
#!/bin/bash
# PVE_HWPATCH-v4 —— 输出节点硬件概要 JSON（单行，纯 ASCII 数值，单位由前端补）
...
EOS
  chmod +x "$SH"; changed=1; echo "  [1] 已写 $SH（v4，纯 ASCII 数值 + LC_ALL=C）"
else
  echo "  [1] $SH 已是 v4，跳过"
fi
...
# ---------- 3) 前端：插入概要条目 ----------
if ! grep -q "$MARK" "$J" 2>/dev/null; then
...
items = """            textField: 'pveversion',
        ...
        },
        {
            ...
            title: gettext('CPU各核'),
            textField: 'tdata',
            renderer: function (v) { try { return JSON.parse(v).cpu_cores + ' \\u00b0C'; } catch (e) { return '-'; } },
        },
        {
            itemId: 'hw-board',
            colspan: 1,
        ...
        },
        {
            ...
            title: gettext('风扇转速'),
            textField: 'tdata',
            renderer: function (v) { try { let f = JSON.parse(v).fans; return f === '-' ? '-' : f + ' RPM'; } catch (e) { return '-'; } },
        },
        {
            itemId: 'hw-disktemp',
            colspan: 2,
...
s = re.sub(r"(alias: 'widget\.pveNodeStatus',\n\n    height: )\d+(,)", r"\g<1>540\g<2>", s, count=1)
open(p, 'w', encoding='utf-8', errors='surrogateescape').write(s)
print("  [3] pvemanagerlib.js 已插桩")
PY
  [ $? -ne 0 ] && { echo "  前端插桩失败"; cp -a "$BK/pvemanagerlib.js.bak.hwpatch" "$J"; exit 1; }
  ...
  echo "  [3] pvemanagerlib.js 已含补丁，跳过"
fi
